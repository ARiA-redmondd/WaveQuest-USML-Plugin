...
BOOST_AUTO_TEST_CASE(quad_boundary_test){
  cout << "=== boundary_test: quad_boundary_test ===" << endl;
  quadtree* tree = new quadtree(0.0,USML_TEST_DIR "/types/test/wedge_cartesian.txt");
  tree->populate_tree();
  cout << "Built quadtree" << endl;
  ...
  reflect_loss_model* reflection = new reflect_loss_constant(0.0);
  data_grid_quad<double,2>* grid = new data_grid_quad<double,2>(tree,-200,3*8001,boundary_points);
  cout << "Built data_grid_quad" << endl;
  boundary_grid_quad<double,2>* boundary = new boundary_grid_quad<double,2>(grid, reflection);
  cout << "Built boundary_grid_quad" << endl;
  
  std::vector<double> interp_point = tree->forward_transform(0.0,0.0,0.0,-200);
  wposition1 pos(interp_point.at(1),interp_point.at(2),interp_point.at(0));
  interp_point.erase(interp_point.begin(),interp_point.end());
  double rho = 0;
  boundary->height(pos,&rho);
