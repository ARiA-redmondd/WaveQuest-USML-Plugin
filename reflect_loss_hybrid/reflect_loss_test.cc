...
BOOST_AUTO_TEST_CASE( hybrid_reflect_loss_test ){
  cout << "=== reflect_loss_test: hybrid_reflect_loss_test ===" << endl ;
  ...
  freq_arr[1] = 5000;
  freq_arr[2] = 12000;
  seq_data freq(freq_arr,3);

  double grazing1 = (4*atan(1)/180)*12;
  double grazing2 = (4*atan(1)/180)*8;

  boost::numeric::ublas::vector<double> phase(3);
  boost::numeric::ublas::vector<double> amplitude(3);

  reflect_loss_model* reflection = new reflect_loss_hybrid(7.5,1500.0);

  reflection->reflect_loss(points,freq,grazing1,&amplitude,&phase);
  BOOST_CHECK_CLOSE(amplitude[0],1.75145,1e-2);
  BOOST_CHECK_CLOSE(amplitude[1],15.7595,1e-2);
  BOOST_CHECK_CLOSE(amplitude[2],30.0,1e-2);
  BOOST_CHECK_CLOSE(phase[0],4*atan(1),1e-2);
  BOOST_CHECK_CLOSE(phase[1],4*atan(1),1e-2);
  BOOST_CHECK_CLOSE(phase[2],4*atan(1),1e-2);

  reflection->reflect_loss(points,freq,grazing2,&amplitude,&phase);
  BOOST_CHECK_CLOSE(amplitude[0],0.809025,1e-2);
  BOOST_CHECK_CLOSE(amplitude[1],12.324375,1e-2);
  BOOST_CHECK_CLOSE(amplitude[2],30.0,1e-2);
  BOOST_CHECK_CLOSE(phase[0],4*atan(1),1e-2);
  BOOST_CHECK_CLOSE(phase[1],4*atan(1),1e-2);
  BOOST_CHECK_CLOSE(phase[2],4*atan(1),1e-2);

  cout << "Computed reflection loss at surface" << endl;
